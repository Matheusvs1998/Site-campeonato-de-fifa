...
const BRAND_RED = '#f30909';
const BRAND_GRAY = '#333333';
...
class MasterOrchestrator extends EventEmitter {
  ...
  async initialize() {
    this._printBanner();
    this._log('Iniciando o Motor de Automação...');
   ...
   */
  _printBanner() {
    const red = chalk.hex(BRAND_RED).bold;
    const gray = chalk.gray;
    
    console.log('\n' + red('  ◈ A I O X') + ' ' + gray('| Autonomous Development Engine v1.0.0'));
  ...
  _log(message, options = {}) {
    ...
    let color;
    switch (level) {
      case 'error': color = chalk.hex(BRAND_RED).bold; break;
    ...
    const prefix = chalk.hex(BRAND_RED)('◈');
    
    console.log(
      `  ${prefix} ${timeStr} ${color(icon)} ${message}`
    );
  ...
  _defaultEpicStart(epicNum, config) {
    const red = chalk.hex(BRAND_RED);
    console.log('\n' + red(`  [EPIC ${epicNum}]`) + ` ${chalk.bold(config.name)}`);
  ...
  _defaultEpicComplete(epicNum, result) {
    ...
    const statusIcon = success ? chalk.green('✔') : chalk.hex(BRAND_RED)('✘');
    const statusText = success ? chalk.green('CONCLUÍDO') : chalk.hex(BRAND_RED)('FALHOU');
    
    console.log(chalk.dim('  ' + '─'.repeat(45)));
    console.log(`  ${statusIcon} Epic ${epicNum} ${statusText}`);
  }
  ...
  _defaultStateChange(from, to, _context) {
    ...
    console.log(gray(`  [STATE] ${from} `) + magenta('➞') + gray(` ${to}`));
  }
}
