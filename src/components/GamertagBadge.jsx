...
import { extractGamertag, extractPlatform } from '../helpers';

const GamertagBadge = ({ fullName, gamertag: directGamertag, platform: directPlatform, style }) => {
    let gamertag = directGamertag || extractGamertag(fullName);
    let platform = (directPlatform || extractPlatform(fullName)).toLowerCase();

    if (!gamertag && fullName) {
        gamertag = fullName; 
    }
    ...
    if (platform.includes('ps')) {
        iconContent = (
            <svg width="15" height="12" viewBox="0 0 200 155" fill="currentColor">
        ...
        iconClass = 'platform-pc';
    } else {
        iconContent = <span style={{ fontSize: '12px' }}>🎮</span>;
        iconClass = 'platform-generic';
    }

    return (
        <div className="gc-gamertag-badge" style={style}>
            <div className="gc-hexagon-outer">
                <div className={`gc-platform-box ${iconClass}`}>
                    {iconContent}
                </div>
            </div>
            <span className="gc-gamertag-text">{gamertag}</span>
        </div>
    );
};

export default GamertagBadge;
